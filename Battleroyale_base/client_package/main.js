...
/*
jcmp.events.Add('battleroyale_render_circles', function(data) {
...
function RenderBarrels(renderer) {
    ...
    for (let i = 0; i < renderBarrels.length; i++) {
        ...
        jcmp.events.CallRemote('battleroyale_debug', 'Rendeing barrel ' + JSON.stringify(barrel));
        renderer.DrawTexture(barrelTexture, new Vector3f(barrel.position.x, barrel.position.y, barrel.position.z), new Vector2f(jcmp.viewportSize.x, jcmp.viewportSize.z));
    }
...
jcmp.events.AddRemoteCallable('battleroyale_render_barrels', function(barrels) {
    ...
    renderBarrels = JSON.parse(barrels);
});*/
